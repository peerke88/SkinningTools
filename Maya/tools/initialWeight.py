...
def jointsToLines(joints):
    ...
    positions = { j: MVector(*cmds.xform(j, query=True, ws=True, t=True)) for j in joints }

    for j in joints:
        point = positions.get(j)
        children = cmds.listRelatives(j, c=True, type="joint", f=True) or []

        childrenData = [ (child, positions.get(child)) for child in children if child in joints and (point - positions.get(child)).length() > 0.001 ]
...
def buildSkinCluster(mesh, joints):
    """
    ...
    # get skin cluster
    sk = shared.skinCluster(mesh, True)
    if not sk:
        # create skin cluster
        sk = cmds.skinCluster( joints, mesh, dropoffRate=0.1 )[0]

    else:
        # make sure all provided joints are an influence of the skin cluster
        # that is already attached to the mesh
        addCleanJoint(joints, mesh, progressBar=None):

    return sk

def setInitialWeights( mesh, joints, iterations=3, projection=0, blend=False, blendMethod=None, progressBar = None):
    """
    ...
    """
    sk = buildSkinCluster(mesh, joints)
    dag = shared.getDagpath(obj)

    lines = jointsToLines(joints)
    connections = shared.getConnectedVerticesMapper(dag)
    points = shared.getPoints(dag)
    components = range(len(points))

    normals = shared.getNormals(dag)
    ...
    utils.setProgress(0, progressBar, "building weights for %s"%mesh )
    for i in components:
        point = points[i]
        ...
        if blend:
            ...
            transformValue = [ [influenceParent, 1 - parameter], [influenceChild, parameter] ]

        cmds.skinPercent(sk, vertex, transformValue=transformValue)
        utils.setProgress(i * percentace, progressBar, "processing: %s"%vertex )
    utils.setProgress(100, progressBar, "build setup for %s"%mesh )
