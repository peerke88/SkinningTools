from shared import *
from joints import comparejointInfluences, removeBindPoses
from mesh import shortestPolySurfaceCurvePathAverage, edgesToSmooth
...
def setMaxJointInfluences(objects = None, MaxInfluenceValue = 8, progressBar = None):
    ...
    for percentIteration, singleObject in enumerate(objects):
        toMuchinfls       = getVertOverMaxInfluence(singleObject = singleObject, MaxInfluenceValue= MaxInfluenceValue, notSelect=True) #returns the vertices that have too much influences
...
@dec_undo
def execCopySourceTarget(TargetSkinCluster, SourceSkinCluster,  TargetSelection, SourceSelection, smoothValue= 1, progressBar = None, amount =1):
...
def transferClosestSkinning(objects, smoothValue, progressBar):
    ...
    object1     = objects[0]
    skinCluster = skinCluster(object1)
    ...
    for iteration, object in enumerate(objects):
        ...
        execCopySourceTarget(skinCluster1, skinCluster,  convertToVertexList(object), self.convertToVertexList(object1), smoothValue, progressBar, amount)
        if progressBar:
            progressBar.setValue(percentage*iteration)
            qApp.processEvents()
...
def transferSkinning(baseSkin, otherSkins, inPlace=True, sAs = True, uvSpace = False):
    ...
    skinclusterBase = skinCluster( baseSkin, silent=False )
    ...
    for skin in otherSkins:
        if inPlace:
            ...
            skincluster = skinCluster( skin, silent=False )
            if skincluster == None:
                continue
    ...
    try:
        ...
        if vertexAmount == 2 or isEdgeSelection:
            ...
            for iteration, vertlist in enumerate(baseList):
                shortestPolySurfaceCurvePathAverage(vertlist, skinClusterName, useDistance, weightWindow = weightAverageWindow)
...
def smoothAndSmoothNeighbours( input, both= False, growing = False ):
    ...
    for mesh in meshes:
        skinClusterName   = skinCluster( mesh )
        if both:
...
def hardSkinSelectionShells( selection, progressBar = False):
    ...
    skinClusterName = skinCluster( meshName, True)
    attachedJoints  = cmds.skinCluster(skinClusterName, q=True, inf=True)
