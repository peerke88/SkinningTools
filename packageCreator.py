...
for dirName, __, fList in os.walk(curFolder):
	for file in fList:
		if (not "ThirdParty" in dirName and "package" in dirName) or "test" in dirName.lower() or "commons" in dirName or "tooltips" in dirName or "promotion" in dirName or "qcachegrind" in dirName:
			continue
		if not '.' in file:
			continue
		if file.startswith('.'):
