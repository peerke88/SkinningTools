...
class VertAndBoneFunction(QWidget):
    ...
    def _rstPose_func(self, sender):
        """ reset bind pose position of joint(s)

        :param sender: button object on which the checkbox is attached
        :type sender: QPushButton
        """
        interface.resetPose(sender.checks["reset bindPose node"].isChecked(), self.progressBar)
